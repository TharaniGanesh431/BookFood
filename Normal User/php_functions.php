...
function add_food($canteen,$food_id,$food_name,$session,$food_price,$food_qty)
{
    ...
    if($photo_upload_confirm)
    {
        ...
        $query = "INSERT INTO food VALUES('$food_id','$food_name',$food_price,$food_qty,'$uploads_dir1',$food_status,'$session','$canteen')";
        $result=mysqli_query($connect,$query);
        if(!$result)
        {
            die(mysqli_error($connect));
...
function view_food_details($canteen, $food)
{
    global $connect;
    ...
    $query = "SELECT * FROM food where food.canteen_id='$canteen' and food.food_name='$food'";
    $result=mysqli_query($connect,$query);
    if(!$result)
    {
        die(mysqli_error($connect));
    ...
    else
    {
        ...
        if(!$row){
           echo '<small class="text-center text-success">Invalid credentials. Please try again later</small>';
            ...
            $food_avail_qty = $row['food_avail_qty'];  
            $food_status = $row['food_status'];  
            $food_start_time = $row['food_start_time'];  
            $food_end_time = $row['food_end_time']; 
            echo "ID : " . $food_id . "<br>";
            echo "NAME : " . $food_name . "<br>";
            echo "PRICE : " . $food_price . "<br>";
            echo "QUANTITY : " . $food_avail_qty . "<br>";
            echo "STATUS : " . $food_status . "<br>";
            echo "START TIME : " . $food_start_time . "<br>";
            echo "END TIME : " . $food_end_time . "<br>";
        }
...
function delete_food($canteen,$session,$food)
{
    
    global $connect;
    $query="DELETE FROM food where food.canteen_id='$canteen' and food.session='$session' and food.food_name='$food'";
    $result=mysqli_query($connect,$query);
    if(!$result)
    {
        die(mysqli_error($connect));
    ...
    {
        echo '<small class="text-center text-success">Food Deleted Successfully</small>'; 
    }
}
